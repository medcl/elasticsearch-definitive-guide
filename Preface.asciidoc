...
它存在的目的就是为了重新赋予硬盘中原始数据新的活力。

无论你是需要全文搜索、结构化数据的实时统计，还是两者结合，这本指南都会帮助你了解其中最基本的概念，
...
这本书适合初学者和有经验的用户。我们希望你有一定的编程基础，虽然不是必须的，但有用过 SQL 和关系数据库会更佳。
...
如果按套路出牌，Elasticsearch 集群扩容分分钟。不过你需要知道如何在进入生产环境前搭建一个稳定的集群，还要能在凌晨三点钟能识别出警告信号，以防止灾难发生。
...
他们照顾着我们和爱着我们，毫不懈怠，忍受我们的缺席和我们没完没了的抱怨这本书还要多久完成，最重要的是，她们还在这里。
